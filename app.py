...
if st.session_state.results:
    ...
    for carrier, results in st.session_state.results.items():
        ...
        ranking_data.append({'Carrier': carrier, 'Services': results.get('service_type', 'Unknown'), 'Raw Score': results.get('total_raw', 0), 'Weighted Score': results.get('total_weighted', 0), 'Weighted %': results.get('weighted_percentage', 0), 'Pending Reviews': pending})
        ...
        for tab_key, tab_res in res.get('tabs', {}).items():
            with st.expander(f"📁 {tab_res['name']}"):
                for q_key, q_data in tab_res.get('questions', {}).items():
                    ...
                    st.markdown(f"**{q_key}**: {q_data['description']} {tier_icon}")
    ...
    if items:
        ...
        for item in filtered:
            ...
            st.write(f"Response: {item['value'][:200]}..." if len(item['value']) > 200 else f"Response: {item['value']}")
    ...
    if st.button("📊 Download Excel Report", type="primary"):
        ...
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            ...
            for carrier, res in st.session_state.results.items():
                ...
                for tab_key, tab_res in res.get('tabs', {}).items():
                    for q_key, q_data in tab_res.get('questions', {}).items():
                        data.append({'Section': tab_res['name'], 'Question': q_key, 'Description': q_data['description'], 'Score': q_data['raw_score'], 'Max': q_data['max_pts'], 'Justification': q_data['justification']})
